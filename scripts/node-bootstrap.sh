...
#     --awg-regen      migrate this node's AmneziaWG obfuscation dialect from the old committed constant to
#                        its PER-NODE derived dialect (Audit-0008 S1-4). Surgical: rewrites only the 9
#                        [Interface] obfuscation lines in the live awg0.conf + every client .conf, keeping
...
#     --awg-issue NAME issue (or RE-issue) a COMPLETE ready-to-import AmneziaWG client config on this live
#                        node: the node mints the client keypair + PSK, enrols the [Peer], and renders the
...
#     --awg-revoke-peer PUBKEY
#                        revoke ONE AmneziaWG peer by its public key. The only way to reach a [Peer] with
#                        no "# name =" marker — such blocks exist on real nodes and a by-name revoke
#                        cannot see them. Same order (interface, config, backups); no client material.
...
# ---------------------------------------------------------------------------
MODE="bootstrap"            # bootstrap | update | ack | revoke | disable-two-hop | node-apply | awg-regen | awg-rotate | awg-issue | awg-revoke | awg-revoke-peer | rotate | rotate-arm | rotate-disarm | rotate-enable-loop | rotate-disable-loop | measure-enable | measure-disable | measure-configure | l7-probe | l7-probe-awg | l7-probe-xhttp | pathsig-probe | fp-probe
...
AWG_REVOKE_PUB=""          # with --awg-revoke-peer: the public key of a single peer to remove (reaches unnamed blocks)
...
verify_post_apply() {
	...
	# vless-xhttp-tls rides the SEPARATE xray engine (not in $SINGBOX_CONFIG); a sing-box client cannot dial
	# xhttp, so its acceptance is L4-only without this: an openssl own-cert loopback handshake against the
	# xray xhttp-tls port (RP-0014 chunk A). Advisory, its OWN marker, never rolls back. No-op if xray absent.
	measure_l7_probe_xhttp "$STATE_DIR/l7_xhttp_postapply.json" || warn "post-apply L7 xhttp-tls liveness flagged a dead outer-TLS layer (marker: $STATE_DIR/l7_xhttp_postapply.json) — the xray xhttp-tls listener is bound but a real client would fail the TLS layer."
	return 0
}
