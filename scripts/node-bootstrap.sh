...
# verify_transports_l7 — L7 acceptance for the CLIENT-FACING transports. verify_listen_ports is L4-only:
# a bound listener can still be client-DEAD (a REALITY donor that breaks the handshake-steal — e.g. the
# 2026-07-01 www.microsoft.com regression — an expired/unreadable own cert, an engine mismatch). For each
# enabled reality/ws-tls inbound in the LIVE config, spin an EPHEMERAL matching sing-box CLIENT against
# the node's OWN loopback listener (the live server), with the SAME creds + uTLS fingerprint a real client
# uses, and confirm a request traverses the tunnel to a neutral 204 endpoint. Records a marker with any
# dead transports and returns non-zero iff >=1 is dead. Best-effort (missing engine/jq/curl -> pass, never
# a false failure). Self-cleaning.
