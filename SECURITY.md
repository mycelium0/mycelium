...
Mycelium is a persistent private network that protects **real people** for whom maintaining private
connectivity is itself restricted in some jurisdictions. A vulnerability in Mycelium is
therefore not "a software bug" — it is a potential **deanonymisation, seizure, or legal
proceeding** for a user or operator. The cross-cutting principle of the project is that **user
safety is functional requirement 1** ([README](README.md), principle 5).

This document describes: what is in scope, how to report a vulnerability privately, the protections
...
risk axes ([refactoring.md §7](docs/refactoring.md)): **user safety**, **availability /
unblockability**, and **anonymity / privacy**.

### 1.1. In scope (priority — by severity, see [§5](#5-severity-classification))

- **User deanonymisation** — any means of linking a user's identity / location to their traffic,
  ingress point, or the fact of their using the tool
  (`USER_DEANON`, **S0**).
...
- **Silent degradation** — a fallback that trades anonymity/indistinguishability for availability
  without an explicit policy (fail-closed violation, `SILENT_DEGRADATION`, **S0**).
...
- **Supply-chain compromise** — a poisoned dependency, upstream substitution, or non-reproducible
  build ([dependency-policy.md](docs/dependency-policy.md)). For a persistent private network this
  is especially critical: one poisoned dependency = mass deanonymisation.
...
- Reports that amount to "a VPN can be blocked" or "traffic can be throttled" — this is a
  **known and fundamental** property ([THREAT-MODEL.md "What the project does NOT promise"](docs/THREAT-MODEL.md)).
  Mycelium promises fast recovery and path redundancy, not absolute unblockability. A report has
...
> **A boundary worth understanding.** Mycelium can reduce the *technical* and *operational* risk
> for a user, but **cannot make them legally compliant inside a jurisdiction that restricts
> private networking** — that is beyond the power of code. Legal exposure is not a "vulnerability"
> in the sense of this document; detailed legal and compliance analysis is maintained in the
> maintainers' internal knowledge base.
...
- Whether the finding exposes distinguishability for **mass** blocking/deanonymisation (this raises
  priority).
...
## 3. Safe harbour for researchers

We want good-faith researchers to help protect users without fear of retaliation. If you act in
good faith and within this policy, the project:

- **Will not initiate** legal action or claims against you for the research itself and the private
  report, provided both are conducted in accordance with this document.
- **Treats your actions as authorised** to the extent described below, and does not consider them
  a breach of our terms.
- **Works with you in a coordinated manner** and, on request, will acknowledge your contribution
  ([§7](#7-public-disclosure-and-credit)).
...
- **cause no harm to users, operators, or third parties**: do not touch live nodes with real users;
  ...
  an isolated environment for PoC;
- **minimise impact**: no more than is needed to demonstrate the finding; no degradation of
  service, data deletion, persistence, lateral movement, or exfiltration;
- do not use the finding for extortion and do not trade it;
- comply with applicable law (see caveat below).

**Honest limits of safe harbour (we do not mislead you):**

- Safe harbour is a **project commitment**, not a state indemnity. The project **cannot** guarantee
  protection from actions by third parties or authorities; it only commits to not pursuing you
  itself for good-faith research. The law of a specific jurisdiction (including network-access and
  CFAA-like statutes) may characterise actions differently than we do — that is outside the
  project's power.
- Attacking infrastructure that **the project does not own** (hosting provider, CDN, upstream,
  a third-party volunteer node) does **not** fall under this safe harbour — those parties have
  their own rules and jurisdictions.
- Violating the conditions above (harming users, publishing PoC before the fix, exfiltration)
  **voids** the safe harbour.

If you are unsure whether your scenario falls within the safe harbour, **ask in advance** via the
channel in [§2](#2-how-to-report), before acting.
...
> **Anti-coercion corollary.** These invariants are not only a design matter but also a coercion
> matter: what does not exist (logs, backdoor, a single central kill switch) cannot be compelled
> from an operator or activated under duress. This aligns with the mere-conduit / no-logs posture
> described in the maintainers' internal knowledge base.
...
It is built around the three project risk axes (user safety / unblockability / anonymity), not
"code criticality".

| Severity | Description | Examples | Primary-response SLA |
|---|---|---|---|
| **S0 — Critical** | Critical violation of safety, unblockability, or anonymity | `USER_DEANON`, `TRAFFIC_CORRELATION`, `DISTINGUISHABLE_TRANSPORT`, `SECRET_LEAK`, `SILENT_DEGRADATION`, `SINGLE_POINT_OF_BLOCK`, `CUSTOM_CRYPTO`, RCE on node/client, poisoned dependency | see [§6](#6-disclosure-timeline) |
| **S1 — High** | Serious risk without immediate deanonymisation; auto-rotation flapping as a signal; `ENUMERATION_EXPOSURE`; `THREAT_MODEL_DRIFT`; `REDUNDANCY_COLLAPSE` | partial ingress enumeration; hardening defect with a real exploitation vector | see [§6](#6-disclosure-timeline) |
...
- **Active exploitation in the wild**, or a finding that hands an adversary a ready signature for
  **mass deanonymisation/blocking** — maximum priority; fix and user notification are prepared in
  emergency mode.
...
- We ask reporters **not to publish exploitation details before the fix is released** — precisely
  because a 0-day in a persistent private network directly harms people. This is the only reason
  for any delay, not an attempt to suppress the issue.
...
- **Credit at the reporter's discretion:** we are happy to acknowledge you (name/handle/link) or
  keep you anonymous — whichever you prefer. For a project that protects private connectivity,
  respect for your own opsec is part of the culture.
...
- **Exact licence text** (`LICENSE` / `NOTICE.md`) and the **safe-harbour clause** in its
  final wording — synchronised with the licensing ADR
  (`docs/adr/NNNN-licensing.md`).
...
A vulnerability in Mycelium is measured by **risk to a human, not "code criticality"**. A finding
that exposes identity, location, the fact of using the network, ingress reachability, or that hands
an adversary a signature for mass blocking is priority 1, however "small" it may look technically. If
you are unsure whether something is a bug, **report it**: for a project that protects people, a
false alarm is cheaper than silence.
