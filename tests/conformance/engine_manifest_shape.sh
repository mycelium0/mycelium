...
#   1. be a JSON object with CLOSED top-level keys (_comment? / version / engines);
...
if [ -z "$(jq -r 'keys[] | select(. != "_comment" and . != "version" and . != "engines")' "$MF" 2>/dev/null)" ]; then
	ok "top-level keys are closed (_comment? / version / engines)"
