...
# myceliumctl — strictly worse than the accumulation this mirror replaced.
#
# THIS ROW WAS WRITTEN, THEN LOST. It went in with the fix, was reverted during a bisect, and never came
# back — so v0.2.103 shipped the guard with nothing holding it, while the CHANGELOG said otherwise.
# Audit-0015 found that by reverting the fix and watching the full suite stay green. Restored here, and
# driven with a RECORDER rather than for real: the refusal IS the absence of an `rm`, and the trace shows
# it directly without mutating a filesystem.
# ---------------------------------------------------------------------------------------------------
...
if grep -q '^rm -f ' "$ETRACE" 2>/dev/null; then
	badln "the tooling was pruned against an artifact that does not even contain myceliumctl ($(grep -c '^rm -f ' "$ETRACE") removal(s) issued). MYCTL then points at a deleted file and the node cannot converge at all."
else
