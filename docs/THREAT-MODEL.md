...
| **The forge as an observation point and a single source** | control plane / node operators (asset #4) | Every armed node pulls the same public repo from its own public address on a fixed cadence, so the hosting forge accumulates a live, current list of the population's public IPs — obtainable by provider pressure or legal process, and outside the operator's control. It is also the only artifact source: while it is unreachable from a node, no operator-pushed change reaches that node at all. Bounded rather than mitigated today: the node keeps serving its last known-good config and its local loops keep running, so the loss is change delivery, not availability; the calendar trigger's jitter spreads ticks only within one quarter hour. **Open:** a second independent artifact source with the signature gate preserved, fetching through an indirection, and per-node cadence offsets |
