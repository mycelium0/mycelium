...
| **D4** | One-command deploy + revoke-without-reinstall + reproducible deploy | **partial** | One-command bootstrap (`scripts/node-bootstrap.sh`) proven on 3 nodes (AF_NETLINK fix at HEAD); Terraform deferred (ADR-0020 §4). **Revoke (corrected):** `myceliumctl identity revoke` removes the client from `identities.json`, but a **re-render + engine reload** is required to take effect (UUIDs are baked into the inbound users) — only a node *reinstall* is avoided. **From-zero deps (done, 2026-06-15):** node-bootstrap now auto-installs the base packages (git/jq/iptables/ufw + curl/tar/unzip, `install_base_deps`) and builds the AmneziaWG userspace tools (amneziawg-go + awg/awg-quick, pinned source) on bootstrap (`install_awg_tools`), closing the manual-fixup gaps a fresh VPS hit. **Missing:** an atomic on-node revoke wrapper (autonomous); the fresh-VPS Ansible/bootstrap-from-zero validation run that exercises the above (operator). | autonomous + operator |
...
4. **Ansible-from-zero** — run the full `infra/ansible` playbook (or `node-bootstrap.sh`) on a freshly provisioned VPS; confirm every role converges; record the transcript. (The from-zero manual-fixup gaps are now closed in node-bootstrap — `install_base_deps` + `install_awg_tools` — so the run should converge without hand-fixups; this run is the validation.)
