...
- **Status:** **IMPLEMENTED** — C4a planner + C4b dry-run seam + C4c-1 gated live apply + C4c-2 disabled unattended timer + C4c-3 live drill all landed (see §5.1). Remaining for full autonomy: the MEASURE plane feeding fresh `rotate_plan.json` from on-node detector/tuner signals (a later RP-0010 chunk); until then the loop ships disabled and applies operator-supplied plans.
