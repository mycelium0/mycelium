...
- **Status:** **IMPLEMENTED** — C4a planner + C4b dry-run seam + C4c-1 gated live apply + C4c-2 disabled unattended timer + C4c-3 live drill all landed (see §5.1). The MEASURE plane now feeds a fresh `rotate_plan.json` from on-node detector/tuner signals (`refresh_rotate_plan_from_daemon`, RP-0010 C5c-3, staleness-guarded), so the loop **self-drives** — an armed node completed an autonomous rotation on a live drill. What remains by design, not by omission: the unattended timer still **ships disabled** (`measure_daemon_ships_disabled`), so autonomy is an explicit, revertible operator opt-in.
...
- **C4c-2 — disabled unattended timer (LANDED).** `mycelium-rotate.service`/`.timer` + `--rotate-enable-loop` / `--rotate-disable-loop` (`rotate_enable_loop`/`rotate_disable_loop` in `nb_rotate_apply.sh`). The timer SHIPS DISABLED — never installed/enabled by `flow_bootstrap`/`flow_update`/`install_tooling`; the explicit `--rotate-enable-loop` is the only path that arms it, and `--rotate-disable-loop` reverts it. Even enabled, each tick only actuates while the node is armed (`--rotate-arm`) with a `rotate_plan.json` present; an un-armed tick is a dry-run. Producing fresh plans from on-node signals is the later MEASURE-plane chunk. `rotate_apply_gated` now pins: no auto-run path references the timer, every `systemctl enable mycelium-rotate` site is inside `rotate_enable_loop`, the disable path exists, and no cron schedules `--rotate`. Verified live on m1 (enable→active→disable→removed).
