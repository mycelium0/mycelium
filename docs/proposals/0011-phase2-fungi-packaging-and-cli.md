...
# RP-0011: Phase-3 fungi-role packaging + deployment/management CLI

## Metadata
- **ID:** RP-0011
- **Slug:** `phase2-fungi-packaging-cli`
- **Status:** **active** (Phase-3 **Operability & Release** track — the operability/release program that, under the 2026-07-02 re-phasing, moved out of the narrowed single-node-adaptivity Phase 2 into the new Phase 3 (Living node: recovery, release, and the fungi/advisory inert seam); the signed Phase-1 GO is recorded in [phase1-acceptance-ledger.md](../phase1-acceptance-ledger.md)). Scope **expanded 2026-06-20** to the full operability program — the unified node model, the installer + management CLI, node diagnostics + a redacted bug-report bundle, and the CI/release/badges surface — sibling to the cross-cutting Advisory Network Awareness increment ([ADR-0030](../adr/0030-advisory-network-awareness.md)); **no extra ladder number for the bridge work** (the ADR-0030 no-ladder-number precedent).
- **Phase:** Phase 3 — **Operability & Release track** ([ROADMAP](../ROADMAP.md) Phase-3 deliverable): the package + installer + management CLI + diagnostics + CI, plus the introduction *mechanism* (inert seam); the live anastomosis *bridge runtime* is exercised at the Phase-3 → 4 boundary and matures in Phase 4–5
...
Phase 3 needs Mycelium to become **deployable and manageable by others as a release** — a package with a minimal **deployment + management CLI** — so that additional **fungi** can be stood up and **anastomosis** can be tested at the Phase-3 → 4 boundary. This is the concrete enabler for the federation layer: you cannot test communes joining until a second operator can stand up a fungi from a release in minutes.

It is also the natural home for the maturing Go spine: **continue the shell → Go migration ([RP-0008](0008-go-spine-distribution-rendering.md) P3) through Phase 3**, so the management CLI is the Go-spine binary (already render-time-resident on nodes since RP-0008 P3 chunk 1), not new bash.
...
**Deferred (Phase 3 → 4 boundary / Phase 4–5):** the live anastomosis bridge runtime (FB ↔ FC carrying traffic, FA stepping away), per the trajectory discipline (ROADMAP / MYC-F006). Phase 3 proves the package, the CLI, and the introduction *mechanism*; the federation *runtime* is exercised at the boundary and matures next.
...
The operator directive of 2026-06-20 expands this RP from "package + CLI" to the full **Operability & Release** track — making a node *one form* that a second operator can deploy and shape in minutes, and making the project's public surface trustworthy and contributable. This is the **Phase-3 release deliverable**, a **sibling** of the cross-cutting Advisory Network Awareness increment ([ADR-0030](../adr/0030-advisory-network-awareness.md)); it does **not** spawn its own extra ladder number (the ADR-0030 precedent: an extra ladder number for bridge work invites actuation drift and churns the Phase 4–6 fences for zero benefit — **rejected** there, and the same reasoning holds here).
...
- **AC-4 (anastomosis survives the introducer — boundary test):** two fungi introduced via the CLI form a direct bridge; turning the introducer off leaves the bridge intact. (Exercised at the Phase-3 → 4 boundary.)
