...
# RP-0011: Phase-2 first-release fungi-role packaging + deployment/management CLI

## Metadata
- **ID:** RP-0011
- **Slug:** `phase2-fungi-packaging-cli`
- **Acceptance:** scored in [rp0011-acceptance-ledger.md](../rp0011-acceptance-ledger.md) (2026-08-09 — **NOT ACCEPTED**: 5 met, 1 partial, 2 unmet, 2 deferred). This RP was `active` with no ledger behind it for its whole life; three criteria were being read as met when they were not.
- **Status:** **active** (Phase-2 **Operability & Release** track — the operability/release program that closes the first-release milestone of Phase 2. (Historical: the 2026-07-02 re-phasing had briefly moved this program out of the narrowed single-node-adaptivity Phase 2 into a separate new Phase 3 (Living node: recovery, release, and the fungi/advisory inert seam); **Decision B (2026-07-04) superseded that** — the Living-node phase dissolved, its recovery/release/operability work folding back into the closing first-release milestone of Phase 2 and its fungi/advisory inert seam becoming Phase-2 groundwork that goes live in the new Phase 3, intra-Commune hypha.) The signed Phase-1 GO is recorded in [phase1-acceptance-ledger.md](../phase1-acceptance-ledger.md)). Scope **expanded 2026-06-20** to the full operability program — the unified node model, the installer + management CLI, node diagnostics + a redacted bug-report bundle, and the CI/release/badges surface — sibling to the cross-cutting Advisory Network Awareness increment ([ADR-0030](../adr/0030-advisory-network-awareness.md)); **no extra ladder number for the bridge work** (the ADR-0030 no-ladder-number precedent).
- **Phase:** Phase 2 — **Operability & Release track** ([ROADMAP](../ROADMAP.md) Phase-2 closing first-release milestone): the package + installer + management CLI + diagnostics + CI, plus the introduction *mechanism* (inert seam, Phase-2 groundwork that goes live in Phase 3); the live cross-Commune anastomosis *bridge runtime* is exercised at the Phase-4 → 5 boundary and matures in Phase 5–6
...
The Phase-2 first-release milestone needs Mycelium to become **deployable and manageable by others as a release** — a package with a minimal **deployment + management CLI** — so that additional **fungi** can be stood up and **anastomosis** can be tested at the Phase-4 → 5 boundary. This is the concrete enabler for the cross-Commune federation layer: you cannot test communes joining until a second operator can stand up a fungi from a release in minutes.

It is also the natural home for the maturing Go spine: **continue the shell → Go migration ([RP-0008](0008-go-spine-distribution-rendering.md) P3) through the Phase-2 first-release milestone**, so the management CLI is the Go-spine binary (already render-time-resident on nodes since RP-0008 P3 chunk 1), not new bash.
...
**In scope (Phase-2 first-release milestone):**
...
**Deferred (Phase 4 → 5 boundary / Phase 5–6):** the live cross-Commune anastomosis bridge runtime (FB ↔ FC carrying traffic, FA stepping away), per the trajectory discipline (ROADMAP / MYC-F006). The Phase-2 first-release milestone proves the package, the CLI, and the introduction *mechanism*; the cross-Commune federation *runtime* is exercised at the boundary and matures next.
...
The operator directive of 2026-06-20 expands this RP from "package + CLI" to the full **Operability & Release** track — making a node *one form* that a second operator can deploy and shape in minutes, and making the project's public surface trustworthy and contributable. This is the **Phase-2 first-release deliverable**, a **sibling** of the cross-cutting Advisory Network Awareness increment ([ADR-0030](../adr/0030-advisory-network-awareness.md)); it does **not** spawn its own extra ladder number (the ADR-0030 precedent: an extra ladder number for bridge work invites actuation drift and churns the Phase 3–5 fences for zero benefit — **rejected** there, and the same reasoning holds here).
...
- **AC-4 (anastomosis survives the introducer — boundary test):** two fungi introduced via the CLI form a direct bridge; turning the introducer off leaves the bridge intact. (Exercised at the Phase-4 → 5 boundary.)
