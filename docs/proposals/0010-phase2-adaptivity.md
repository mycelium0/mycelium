...
The selection is driven by the **ADOPTed reinforce-and-evaporate scoring law** — standard, named prior art: **EWMA / exponential-smoothing** reinforcement + **exponential time-decay** + **control-theory Schmitt-trigger hysteresis** (the Physarum/Tero-2010 / ant-colony imagery is the bio-inspired **metaphor** for the decay-and-scar intuition, not the literal implemented equation) — expressed directly on the existing `spec.DecayPolicy`: per `(transport-class, path)` keep a weight `W` that is **reinforced** on observed reachability/goodput and **decays continuously** (`HalfLife`) so a just-degraded shape *fades without explicit teardown* and *re-promotes automatically* when a block lifts; `Hysteresis` damps flapping; `RetentionFloor` is "scar memory" so a repeatedly-blocked shape is not eagerly retried. This is a few-line scoring update — **not** a routing/discovery protocol.
...
- **AC-6 (no new measurement surface):** detection is fed from `internal/reach` only; no new active-probing fingerprint is introduced (the WRAP, not a rebuild).
  - **Clarification (2026-07-02 — reconciling AC-6 with the Plane-2 signal list).** "No new active-probing fingerprint" means no new **external / client-vantage** fingerprint: no synthetic request egressed to a third party on a cadence (fetching a fixed `/generate_204` *through the tunnel* every tick is forbidden — it is exactly the "fingerprintable probe-protocol signature" VIS-0004 warns against, and a real security regression on a restrictive link). It does **not** forbid a **node-local, loopback, own-keys L7-liveness probe**: completing the channel's own handshake against the node's **own** listener over `127.0.0.1` with the node's **own** credentials, emitting **no external packet** (an authenticated REALITY probe is answered locally; only *un*authenticated scanners are proxied to `dest`). That probe is **not a new measurement surface** — it is the realization of the Plane-2 `active-probe response failure (own-cert / cover path)` signal into the already-typed `spec.DetectorSignal.ActiveProbeOK` field (today stubbed `true` in `internal/measure.detectorSignal`), it keeps reach's node-local posture ("own-listener probes, not client-vantage"), and it MUST obey the **hyphal-probe** invariants (VIS-0004: budgeted, jittered cadence, bounded, no unbounded crawl) so it is never a periodic beacon. Division of labour: **inbound L7-death** (broken REALITY `dest` / bad own-cert — the reach L4-only blind spot) is caught by this loopback probe; **egress / throughput death** stays the passive by-product signal (throughput collapse after connect, above). This closes the reach L4-only blind spot (a bound listener that is client-DEAD at L7) without adding any external fingerprint.
...
- **C3 — the self-tuner (LANDED).** `internal/tune`: a reinforce-and-evaporate scoring law — EWMA/exponential-smoothing + exponential time-decay + control-theory Schmitt-trigger hysteresis (Physarum is metaphor, not the implemented equation) — on `spec.DecayPolicy`, as a per-`(transport-class, path)` `Weight` — each good `Verdict` reinforces it; it decays by `HalfLife` toward `RetentionFloor` (scar memory), so a blocked shape fades without teardown and re-promotes automatically on recovery; a `Hysteresis` band damps the promote/demote flag (AC-3, proven by `TestFadeAndRePromoteWithoutTeardown`). Fail-closed `NewWeight`; a ranking input only — never actuates (AC-4). Gate `tuner_pure_advisory` enforces purity (no net/os/syscall, no reach/detect coupling). Still inert: nothing consumes the ranking yet.
- **Plane-1 MEASURE realization (LANDED) — `internal/measure` + `myceliumd` + node deploy.** Plane 1 was scoped as "WRAP, no new code", but the reach→detect→tune→assemble wiring turned out to need a real (still pure) seam: `internal/measure.Assembler` folds a `reach` snapshot through the C2 detector + C3 tuner into a `rotate.PlanInput` (advisory; gate `measure_pure_advisory`); it is embedded in the `myceliumd` daemon (Type=notify; gates `measure_daemon_advisory` + `measure_daemon_ships_disabled`); `install_spine` builds the daemon and `nb_measure.sh` `--measure-enable` ships it DISABLED; `generate_measure_configs` derives ref-matched reach (own-listener probes — a node-local signal, not client-vantage) + measure configs from params/registry; and `flow_rotate` self-drives off the daemon's PlanInput, staleness-guarded, **still behind the RP-0012 triple gate**. **Self-drive drill PASSED on m1 (sandbox, dry-run):** real reach probed m1's live listeners → a measured fault on the active (vision reach → a closed port) → the assembler produced `act=true` to the grpc sibling; clean → HOLD; a stale PlanInput → refused; m1's live config + m2/m3/m4 untouched. *(These chunks were labelled "C5a–c" in commit messages before noticing the `C5 — advisory emit` row below; read them as this Plane-1 realization, a distinct chunk — the running counter collided.)*
