...
- **Status:** **LANDED — node-local complete** (drafted 2026-07-13). All three chunks shipped behind their own gates: **A** (full L7-liveness coverage for every closed family), **B** (the passive path-level interference signal — 1a the nft reset observer, 1b the daemon fold, 1c the gate/e2e-test/drill, plus increment 2 `PostConnectCollapse`, which ships INERT until an on-node arming drill) and **C** (the projection seam `measure.StatusObservations()`). What remains is **externally blocked, not undone**: the live reset drill and the collapse arming both need an operator node target, and the digest live-emit belongs to a future publish RP + Phase-5 federation.
...
- **`PostConnectCollapse` — mechanism RESOLVED by an adversarial design panel (2026-07-17): a passive per-class `/proc/net/tcp{,6}` send-queue-stall observer.** A post-connect collapse happens DOWNSTREAM of the node, so a local egress byte counter cannot see it (the node's egress, including retransmits, succeeds). But it leaves a node-LOCAL kernel signature: an ESTABLISHED served socket whose send backlog (`tx_queue` = `write_seq − snd_una`) stays non-empty AND whose unrecovered-retransmit count (`retrnsmt`) climbs — because `snd_una` advances ONLY on a real inbound client ACK, so retransmits alone never clear it. The observer reads `/proc/net/tcp{,6}` (no new nft rule, no conntrack, no sysctl — cheaper than the parked options (a) conntrack-acct and (b) nft byte-ratio, both of which were rejected: (a) needs a sysctl + package the nodes lack and structurally under-fires the canonical case; (b) counts egress the collapse doesn't stall), gated by the existing increment-1 `syn_<port>` churn counter, and flags a class when a majority of ≥8 concurrent established flows are stuck. It ships **DISARMED** (`path_collapse_enabled` false): the `collapse` marker list is written in SHADOW but the fold is inert until an on-node drill validates the `/proc` field parse (`retrnsmt` is field 7, not the field-8 uid — the false-fire landmine) and the fire/silence behaviour. Superseded options (a)/(b) stay on record only as the noise-corroborator fallback (add an independent nft byte-rate signal and require BOTH to fault) if the drill shows the send-queue signal alone is noisy.
