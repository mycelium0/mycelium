...
- **Status:** active (partially built — W5 template-collapse + the `--update`/`--staged`/`--ack` engine + the systemd units landed; **W1 signed-release pipeline still unarmed** (nodes run `--insecure-no-verify`), W3 network-wide update timer not yet installed, W6 supply-chain gates pending)
