...
| **Hysteria2** | QUIC/UDP with aggressive congestion control; the ONLY family servable across a **port range** (`server_ports` + `hop_interval` on the sing-box outbound — tuic rejects the field, and the hysteria2 *inbound* rejects it too, so the range is delivered by a nat/PREROUTING REDIRECT onto the one served port) | Strong on lossy, throttled, high-latency links; the range is handed to the client UP FRONT, so the served port can move inside it without any client learning anything. **A port block is DILUTED, not removed** — measured on a node (`tests/netsim/scenarios/hop_range_dilutes_a_port_block.sh`): sing-box hops on a TIMER and does not avoid dead ports, so the client is down while it sits on a blocked one and the outage fraction tracks blocked/total (1 of 3 blocked → ~25% of samples gapped; 1 of 11 → ~6%). A wider range dilutes better and widens the observable footprint: the trade is explicit | Depends on live UDP. **A range widens the node's observable port footprint** — see [THREAT-MODEL](THREAT-MODEL.md); off by default, and its width is an operator decision. **Clash-Meta clients get no range at all**
(Audit-0010 F-020): that emitter writes `port:` only, so a Clash client pins the served port and gets
none of the dilution — on a node that has already paid the whole cost, since the REDIRECT is installed
and the widened footprint is standing. The two client formats are NOT equivalent for this feature. The rule that makes a range real is invisible to every node-local check (loopback never traverses PREROUTING), so `verify_hy2_hop_nat` asserts it in the converge tail, *after* the firewall step that installs it | UDP-friendly networks |
...
- **Telemetry and policy:** anonymised block-event signals feed a policy of "which transports
  ...
  - **Params-knob validation (ADR-0038):** every operator-settable params knob whose value is
    *structured* — a port range, a path, a duration — is validated by exactly one owner,
    `internal/spec`, whose bounds and defaults are emitted into `control/vocab.json`
    (`.params_validation`). Shell consumers **compare against the emitted numbers; they never re-derive
    the rule.** The reason is a failure class this project hit directly: when a hop range was judged
    separately by the renderers and by the firewall, a value the renderers accepted and the firewall
    refused produced client configs advertising a range with no rule behind it — and *nothing on the
    node could observe it*, because `verify_post_apply` checks the service, the bind and a **loopback**
    handshake, and loopback traffic never traverses PREROUTING. A corollary is enforced as a class: a
    params key a renderer reads must be reachable by an operator (generated by `write_params`, in the
    Go-owned allowlist, or stamped by a named posture step) — a knob in none of these is not "off by
    default", it is unsettable.
  - **Reserved rotation moves:** `rotate-port` and `regen-reality` are declared, validate and
    round-trip, and the planner must never emit them — both change what a client must DIAL, and unlike
    a demote the client cannot discover the new value from a set it already holds. **One exception,
    stated because the row above contradicts the rule as written:** a hysteria2 client holding a hop
    RANGE does follow a served port that moves *inside* that range, since the range is what it dials.
    Even so `rotate-port` stays reserved — the planner has no way to know a given client holds a range
    (issued configs are not tracked), the mechanism covers exactly one family, and a moved port outside
    the range strands every client regardless. Unreserving it would require the move to be conditional
    on a range being configured AND on the target lying inside it, which is a planner precondition
    nobody has written. The reservation is
    enforced on the **field that causes the move** (`To.ToPort` is zeroed on every act branch), not on
    the action name: the name alone let a port move ride in under `promote-sibling`, because the
    executor applies `.to.to_port` without consulting `.to.action` and port keys survive `write_params`.
