...
AS-level blocking — do not concentrate everything in one tainted AS"*) requires that the network be spread
across **distinct, independently-blockable autonomous systems**, and that this spread be **auditable** — a
single IP/AS block must never take out a disproportionate share of the network, and fresh capacity must be
held in reserve.
...
is the **template + the network's diversity rationale** in abstracted form.
...
## Network diversity invariants (the auditable bar)
- **≥3 distinct autonomous systems** across the network; no single AS hosts a majority of nodes.
...
## Current network (abstracted; real values in the local inventory)
...
**Status (2026-06):** the network runs **3 nodes in 3 distinct countries on 3 distinct autonomous systems**
