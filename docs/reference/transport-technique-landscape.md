...
| 8 | **TLS-fragment advisory field** in the bundle / Inoculum `transport_profiles` (operator-tunable; document as SNI-inspection-only, **orthogonal** to the throughput-throttle finding) | A cheap, complementary anti-SNI-inspection layer; per-network tuning | sing-box 1.12+ / Xray | 1 | **adopt** (narrow, low-pri) | Low | sing-box.sagernet.org |
...
| TCP-fragment + fake-TTL desync (as a node feature) | Client / endpoint behavior against local network interference; Mycelium is node software, not a client (ADR-0016). | github.com/XTLS/Xray-core |
