...
(`*.example.com`) was issued once and **copied to every node**, with the TLS-QUIC transports
(Hysteria2, TUIC) presenting it under SNI `m.example.com` so clients connect by IP with zero
...
- It creates a **central dependency** on one owner's domain (`example.com`) and its renewal.
