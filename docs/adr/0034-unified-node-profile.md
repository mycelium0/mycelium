...
- **Phase:** Phase 2 — **Operability & Release track**, the closing first-release milestone (this unified node descriptor is a packaging/CLI deliverable of the first release, [ROADMAP](../ROADMAP.md) Phase-2 milestone); **not a phase gate**. (History: the 2026-07-02 re-phasing had placed the operability/release/packaging track in a separate new Phase 3 "living node"; **Decision B, 2026-07-04, superseded that** — the Living-node phase dissolved and its recovery/release/operability work folded back into the closing first-release milestone of Phase 2, its fungi/advisory inert seam becoming Phase-2 groundwork that goes live in the new Phase 3, intra-Commune hypha. The [ADR-0030](0030-advisory-network-awareness.md) no-bespoke-ladder-number precedent still holds.)
...
3. **Reachability is a posture on one node form, not a variant.** `reachable: false` means *provisioned and converged, but not a public entry*: public inbounds bind loopback (the existing ShadowTLS-detour pattern) **and** the firewall does not open their ports — privacy held at **both** the bind and firewall layers, fail-closed (never merely "skip opening" a port on a host with no prior firewall). It composes with `ingress`/`front` (a non-public node can still be an egress/relay participant) and preserves the sshd-anti-lockout ordering when the firewall is re-applied. The name for this posture is a **non-public participant** — never a "node variant"; the framing is a population of nodes joining one network (see [GLOSSARY](../GLOSSARY.md)).
   ...
   *Implementation staging (RP-0011 chunk D).* The mechanism is a single render-time `node_bind` param (default `"::"`, byte-identical to today; `apply_node_profile` stamps `"127.0.0.1"` only when the descriptor declares `reachable: false`), applied identically by the shell renderer and `internal/spec.RenderServer` (the `render_server_go_equiv` gate pins them byte-identical, including a `reachable=false` fixture). The **bind layer holds on every flow** (bootstrap / update / `--node-apply`) the instant the descriptor sets `reachable: false` — a loopback-bound inbound is unreachable off-host *regardless* of the firewall, so the posture is never fail-open. The **firewall layer** follows automatically: `harden_ufw`'s loopback exclusion is generalised to all inbound types, so a loopback-bound port is never opened — and since `harden_ufw` runs at bootstrap, the firewall layer converges then. **Amended 2026-07-31 (Audit-0009 I1).** The paragraph above said a live `--node-apply` does **not** re-run the firewall and listed a firewall-only reconverge as a tracked follow-up. That stopped being true when the shared convergence tail landed: `--node-apply`, `--revoke`, rotate, `--disable-two-hop` and the unattended `--update` all end in `converge_node_tail`, which runs `harden_ufw`. So the firewall now re-converges on **every promote path**, in both directions, with the sshd-allow-first ordering preserved — the follow-up is delivered, not pending, and the availability gap it described (public again but ports still closed until the next bootstrap) is closed.

   What the tail must NOT do is decide the posture from an invocation's argv. It read `${DO_HARDEN:-1}`, which is set only from the command line, and the update timer invokes the updater with no flags at all — so a node deliberately bootstrapped `--no-harden` had ufw force-enabled on its first tick, blocking every non-mycelium inbound service on the host while anti-lockout preserved only SSH. The posture is therefore **node state**: a `harden` field on the node profile (`NodeProfile.Harden`, a pointer so absent stays distinguishable from declared-false), falling back to the posture the establishing bootstrap was given, and only then to ON. A declared field outranks the remembered one; a plain `--node-apply` never overwrites it. §3's "both layers" is unchanged — **bind always, firewall on every convergence** — and the **bind layer remains the authoritative privacy control**, never fail-open, including on a node whose firewall posture is off.
