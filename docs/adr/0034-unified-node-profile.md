...
- **Phase:** Phase 2 — **Operability & Release track** (the packaging/CLI deliverable, [ROADMAP](../ROADMAP.md) Phase-2 deliverable (b)); **not a phase gate** and **not a phase renumber** (the [ADR-0030](0030-advisory-network-awareness.md) no-ladder-number precedent applies)
...
2. **Capabilities, not types.** The descriptor's fields are capability layers on **one** node form, each default-off / safe-default, illustrative shape:
   ...
   - `reachable` — the **posture** (see §3); the descriptor **default is off / safe-default** (a freshly-provisioned node does not auto-advertise), set `true` only when the operator declares the node a public entry, and stays `false` for a provisioned-but-not-a-public-entry node.
