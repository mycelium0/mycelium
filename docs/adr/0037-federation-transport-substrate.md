...
# ADR-0037: Federation transport substrate — Nebula (hypha) + libp2p (bridge); inert contract in Phase 2

> Records **one** decision: when node-to-node federation goes **live** (Phase 3+), Mycelium **reuses proven
...
> **Phase 2** only the **inert, substrate-agnostic contract schema** lands: the **hypha** seam is built
> (typed + `Validate()`, zero callers), the **`AnastomosisBridge`** grammar is **declared but Phase-5
...
Phase 2 must land the **inert intra-Commune F2F hypha edge-fusion seam** ([ADR-0026](0026-anastomosis-bridges-and-safe-defaults.md)
Decision 5) — schemas + the introduction mechanism ([ADR-0029](0029-community-federated-ingress.md)
Decision 5) only, **no live federation** ([ROADMAP](../ROADMAP.md) Phase-2 DoD). When federation *does* go
live (Phase 3+), the ROADMAP already commits to **reusing proven substrate** (Nebula / WireGuard-Noise for
mutual-auth node links; private libp2p for NAT'd peers) rather than reinventing it. This ADR pins *which*
substrate, so the Phase-2 inert schema can be shaped to **compose with it without lock-in**, and records the
...
**1. Live substrate (Phase 3+): Nebula for role A, libp2p for role B — reused, not reinvented.**
...
**3. Phase-2 lands only the inert, substrate-agnostic contract** (`internal/spec`, per
...
The contract captures **five substrate-agnostic fields** so it composes with either transport at Phase 3+:
...
4. **consent / double-opt-in** — both sides opt in ([ADR-0029](0029-community-federated-ingress.md)
...
- **BUILT inert (Phase 2):** `IdentityHandle`, `TrafficCapabilityClass`, `CapabilityPolicy`,
...
- **DECLARED, Phase-5 deferred:** the `AnastomosisBridge` 8-term contract grammar
  ...
  visible, but **nothing establishes, negotiates, or propagates a bridge** before Phase 5. Built inert
...
- The Phase-2 inert schema is transport-agnostic: plugging in Nebula (hypha) or libp2p (bridge) at Phase 3+
...
- Inertness is enforced, not merely intended: the new types must land with **zero production callers** and an
  ...
  binds the federation types until Phase 3-5.
- **Risk:** naming substrates now could invite premature wiring. Mitigation: the inertness gate + the hard
  phase split in ADR-0026 Decision 5; building a live publisher/dialer in Phase 2 would *violate* the DoD.
...
- **Yggdrasil / cjdns** — a self-arranging encrypted mesh overlay; reserved for the **Phase 4/6** mesh, not
  the Phase-3 mutual-auth node link or the governed bridge.
- **A homegrown node-link protocol** — rejected outright (ADR-0002 no-custom-crypto, ADR-0031 build-vs-reuse).

## Status

**accepted** — Phase 2 (inert schema; hypha built, bridge declared) / Phase 3+ (live transport). Extends
[ADR-0026](0026-anastomosis-bridges-and-safe-defaults.md) (bridge grammar + capability taxonomy) and
[ADR-0029](0029-community-federated-ingress.md) (introduction mechanism); governed by
