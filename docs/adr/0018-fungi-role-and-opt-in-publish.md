...
4. **Dissolution into mesh-native digest spores (Phase 3-4).** The opt-in upload path is the
   **early transport** for an unchanged public contract. In Phase 3-4 a fungi emits the same
...
5. **Anti-Sybil source weighting — design here, not running.** An open fungi role invites fake
   ...
   - **`import-inert-until-validated`** — every ingested digest is inert until its signature and
     ...
     is the verifier's Phase 3-4 job against a named standard primitive, never a new scheme);
...
6. **Phase discipline.** In **Phases 0-2 the fungi path runs only as opt-in, PII-safe, aggregated,
   ...
   (decision 4) is **Phase 3-4**. This ADR records the role contract and the opt-in path as a
...
- The **public contract is transport-agnostic**: the Phase-0 opt-in upload and the Phase-3-4
...
- **A running anti-Sybil reputation/weighting system now** — deferred: full source weighting is
  load-bearing and belongs with the publisher and the floor/noise ADR (Phase 3-4). This ADR bounds a
...
- **Pulling the mesh-native spore transport into Phase 0-2** — rejected: it would run a DHT / gossip /
  ...
  Phase-0-2 transport; the spore path is Phase 3-4, with the public contract unchanged.
