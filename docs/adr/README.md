...
| [0032](0032-xray-automated-toggleable-engine.md) | Xray as an automated, per-protocol-toggleable engine peer to sing-box — engine diversity is the resilience mechanism (ADR-0028); each node serves only what its toggles enable (default-off, additive — never a mutually-exclusive engine role); the Xray-only `vless-xhttp-tls` (genuine single-TLS own-cert XHTTP) is the one xray-served transport; co-location correlation trade-off recorded (opt-in, diversity wins) | accepted |
| [0033](0033-operator-cdn-front-relay-byod.md) | Operator-self-hosted CDN/ingress front — relay-preferred, bring-your-own-domain, opt-in, default-off. Extends ADR-0029's "CDN layer" as a deploy-time option; frontable only for the genuine-single-TLS HTTP transports {vless-xhttp-tls, vless-ws-tls} (REALITY refused); relay (SNI-passthrough, edge holds no key) is the default and terminate is an ack-gated metadata trade-off; complementary/last-resort against the destination-class throttle but **primary** against a port-keyed filter (443 SNI-passthrough rewrites the discriminator) | accepted |
| [0034](0034-unified-node-profile.md) | Unified node profile — one node-local descriptor where engine/ingress/CDN-front/reachability/weather-opt-in are default-off **capabilities** of one node form, never a node-TYPE enum (fungi is a reversible niche, not a class — ADR-0018) and never a mutually-exclusive engine role (engines stay additive — ADR-0032). Additive + byte-identical for a node that adopts no new field; node-local/never-committed; the `node_profile_single_source` gate; the descriptor RP-0011's CLI writes. Phase-3 Operability & Release track (roadmap re-phasing 2026-07-02) | accepted |
| [0035](0035-diagnostics-bundle-redaction-contract.md) | Diagnostics-bundle redaction contract — the operator `diag collect` bundle is PII-safe by construction: a pure ordered `Redact` (fail-safe by over-redaction, idempotent; covers labelled fields, dial/lookup operands, IP/MAC/FQDN, UUIDs, key material + opaque tokens, ASN) + a `RedactBundle` own-host belt (word-anchored, length-floored) that both verbs route through; a small **named residual** (free-floating unlabelled dot-less sub-8 value) the operator reviews; read-only collector, fail-closed, 10 s subprocess timeout. RP-0011 chunk E / AC-9; the `log_bundle_redaction` gate. Prompted by [Audit-0006](../audits/0006-diagnostics-redactor-pr-audit.md) | accepted |
