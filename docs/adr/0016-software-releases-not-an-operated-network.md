...
> [0015-network-artifact-delivery-and-node-update.md](0015-network-artifact-delivery-and-node-update.md),
...
rejects (ADR-0014: no shared network key material; AGENTS.md §1.4: no permanent central brain), and
...
- The network updater (ADR-0015) stays in its **unsigned/insecure interim** until consensus signing
  lands; the safety of "a push reaches the network" rests on repo write-access control until then.
