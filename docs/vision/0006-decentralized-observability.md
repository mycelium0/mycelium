...
| **L2.5 — compartment wound response** | a compartment seals an infected zone; false seals self-heal | threshold-signed `spec.SporeEnvelope` (hard-class) on k-of-n corroboration | **signalling Phase 3–4; routing actuation Phase 6** (signalling ≠ actuation) |
...
5. **Signal-speed non-escalation** — a fast (`TransportHealth`) or medium (`StressSignal`) signal can never
   ...
   network-level Phase 3+, hard quarantine Phase 6).
