...
- **A TLS-terminating CDN is also a disclosure surface.** A path that terminates TLS at a third party
  hands that party the user's source address *and* the destination hostnames — a identity exposure and
...
- `REACHABILITY_TOPOLOGY_VIOLATION` (**S1**; **S0** if it creates a identity exposure channel) — a
