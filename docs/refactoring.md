...
- sybil resistance of discovery (for phases 4–5);
...
- suspected deanonymization or traffic correlation;
...
phase (in phases 4–5, an error in discovery or sybil resistance exposes the
network map to an adversary).

### Phases 0–2 (single node → multi-protocol → adaptation)
...
### Phases 4–5 (decentralization → autonomous mesh)
...
### Mature network (after phase 5 stabilization)
...
- **Persistence of control plane and bootstrap:** configs, commands,
  ...
  minutes;
- **Sybil / enumeration (phases 4–5):** an adversary cannot cheaply enumerate
...
- **Telemetry/blockage intelligence:** aggregated, noised, not linked to identity;
  does not itself become a deanonymizing channel;
...
- a "convenient" feature that improves UX at the cost of deanonymization
...
- ...
Correlation / deanonymization risks:
- ...
Required findings:
- ...
```
...
three risk axes: **user safety**, **availability/unblockability**, and **anonymity**.

### S0 — Critical
A critical violation of safety, unblockability, or anonymity.
Requires immediate halt of merge / rollout / phase transition.

Examples:
- a node or telemetry exposes the "user ↔ destination" mapping (deanonymization);
...
It is pointless to have an unblockable tunnel if configs, commands, and first
...
As soon as joining the network becomes open (phases 4–5), discovery must resist
