...
having an reachable tunnel if the config that configures it is served from a domain that can
...
8. **Duplicating a source of truth.** One truth type — one owner (see §2.4). Two locations
   ...
   **S1**, **S0** if the divergence leads to an unsafe path selection or identity exposure).
...
identity exposure **does not pass**.
...
- add hidden dependencies or **covert network channels** (callback-home, rogue telemetry) —
  these are a direct identity exposure threat;
...
- creates a single point of blocking or identity exposure,
